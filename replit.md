...
- **Multi-teacher system**: Each teacher has a row in `teachersTable` (id, name, slug, passcode, subject). `timeSlotsTable` has nullable `teacherId` FK. Auth cookie encodes `{teacherId}:{HMAC(secret, teacherId|passcode)}`. Routes: `/` = teacher directory, `/book/:slug` = student booking form for that teacher, `/teacher` = teacher login/dashboard, `/teacher/signup` = new account. First teacher to sign up automatically claims all unclaimed (NULL teacherId) slots. All teacher API routes filter by `res.locals.teacherId` from session.
